...
#endif
static int check_wav_head(struct wav_reader* wr,struct wave_pcm_hdr *wav)
{
	if(strncmp(wav->riff,"RIFF",4))
	{
		printf("wavdata head error wav->riff = %s\n",wav->riff);
		free(wr);
		return -1;
	}
	if(strncmp(wav->wave,"WAVE",4))
	{
		printf("not wav data\n");
		free(wr);
		return -1;
	}
	if(strncmp(wav->fmt,"fmt ",4))
	{
		printf("wavdata head error\n");
		free(wr);
		return -1;
	}
	wr->format=wav->format_tag;
	wr->channels = wav->channels ;
	wr->sample_rate=wav->samples_per_sec;
	wr->bits_per_sample=wav->bits_per_sample;
	wr->data_length=wav->data_size;
	return 0;
}

void* check_wavdata(const char *wavdata) {
	struct wav_reader* wr = (struct wav_reader*) malloc(sizeof(*wr));
	memset(wr, 0, sizeof(struct wav_reader));
	wr->wav=NULL;
	
	if(check_wav_head(wr,(struct wave_pcm_hdr *)wavdata))
	{
		free(wr);
		return NULL;
	}
...
void* check_file(const char *infile) {
	struct wav_reader* wr = (struct wav_reader*) malloc(sizeof(*wr));
	memset(wr, 0, sizeof(struct wav_reader));
	wr->wav=fopen(infile,"r");
	struct wave_pcm_hdr wav;
	fread(&wav,WAV_HEAD,1,wr->wav);
	if(check_wav_head(wr,&wav))
	{
		free(wr);
		return NULL;
	}
...
int wav_read_data_inter(void* obj, char *wav_data,unsigned char** data, unsigned int length) {
	...
	if (length > wr->data_length)
		length = wr->data_length;
#if 0	
	memcpy(data,wav_data+WAV_HEAD+wr->pos,length);	
#else
	*data = (unsigned char*)(wav_data+WAV_HEAD+wr->pos);
#endif
	wr->data_length -= length;
	n = length;
	wr->pos+=n;
